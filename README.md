...
会員登録をしなくても、ログイン画面の「デモ用ログイン」ボタンから、パスワード入力なしで以下2種類のデモアカウントにログインして一通りの機能を試せます。

| 用途 | アカウント |
|---|---|
| 出品者用 | test@example.com |
| 購入者用 | demo@example.com |

デモアカウントのプロフィール（住所など）は変更できないよう保護しており、また閲覧者による出品・購入・コメントなどの変更内容は、毎日0時（Asia/Tokyo）に自動でシード直後の状態へリセットされます（`php artisan demo:reset`をタスクスケジューラに登録）。
...
- **デモ用ログイン機能**（会員登録なしにボタン一つでデモアカウントへログイン、デモアカウントの情報改変を防ぐガード、閲覧者による変更内容を毎日自動でリセットする定期処理）
...
| POST | /demo-login/{type} | デモ用ログイン処理（出品者/購入者を選んでパスワード無しでログイン） |
